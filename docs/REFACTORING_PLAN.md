...
**整个仓库原 v4.2.0 清理目标已全部完成，但仍有若干文件明显超过 600 行软上限（最大超过 1300 行），属于遗留的拆分层面技术债，见下方"后续可选切片"节。**

质量门基线已对齐 CI 实测值：mypy 73（v4.2.0 起 342 → phase 1 清到 150 →
phase 2 清到 72，随后 runner guard 对齐到 73）、ruff pyflakes 169（原 181 → 177 → 172 → 169）、coverage
`fail_under=62`（实测 63.13%，phase 2 portfolio_optimizer 加 35 个特征化测试
后从 62.54% 提到 63.13%），未来 regressions 直接失败。
...
以下文件**当前超过 600 行软上限**（2026-05-22 实测），应在相关功能有新增或维护时优先拆分：

| 文件 | 实测行数 |
|---|---:|
| `frontend/src/components/cross-market/CrossMarketResultsSection.js` | 1331 |
| `backend/app/api/v1/endpoints/alt_data.py` | 1167 |
| `src/analytics/pricing_gap_analyzer.py` | 1136 |
| `frontend/src/components/ResearchWorkbench.js` | 1133 |
| `src/data/data_manager.py` | 1080 |
| `frontend/src/components/research-playbook/playbookViewModels.js` | 1056 |
| `src/data/providers/akshare_provider.py` | 1052 |
| `frontend/src/components/research-workbench/WorkbenchOverviewPanels.js` | 1027 |
| `frontend/src/components/CrossMarketBacktestPanel.js` | 1011 |
| `backend/app/services/quant_lab_alerts.py` | 998 |
| `src/data/alternative/narrative.py` | 953 |
| `src/data/alternative/composite_signal.py` | 916 |
| `src/data/alternative/macro_briefing.py` | 758 |

下面的小切片（600 行以下）只在新增功能或附带性维护时顺手做：
