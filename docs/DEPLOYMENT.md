...
v5 前端位于 `frontend/`（Vite + React 19 + TypeScript + Tailwind v4 + shadcn/ui 暗金），产物输出到 `frontend/dist`。
...
- 开发环境：`frontend/vite.config.ts` 配置 Vite proxy，把 `/api` 请求代理到后端 `:8100`；前端 dev server 运行在 `:3100`
