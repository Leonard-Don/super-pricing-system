...
# ---------------------------------------------------------------------------
# composite-signals theme — relocated to ``alt_data_composite_signals``.
#
# The 4 composite-signals handlers (/composite-signals, /composite-signals/
# history, /composite-signals-cluster-aware, /composite-signal-comparison)
# plus the ``_CONVICTION_TIER_RANK`` constant moved to a sibling module to
# trim this file. They are merged back via ``include_router`` so every route
# stays mounted at its identical ``/alt-data/...`` path with an unchanged
# request/response schema.
#
# The import is deferred to module-end on purpose: the sub-module imports
# this module back (it resolves ``_get_manager`` / ``_get_composite_signal_
# archive`` lazily at request time, so tests can monkey-patch them on the
# ``alt_data`` namespace). Importing at the top would deadlock the circular
# import; importing here means this module is fully defined first.
#
# The composite-signal symbols below are re-exported so anything that used
# to ``from ...alt_data import detect_composite_signals`` keeps working.
# (``COMPOSITE_DEFAULT_CLUSTER_THRESHOLD`` is already imported at the top
# because the still-resident ``/themes-with-diversity`` handler needs it
# as a module-load-time Query default.)
from backend.app.api.v1.endpoints import (  # noqa: E402
    alt_data_composite_signals as _alt_data_composite_signals,
)
from src.data.alternative.composite_signal import (  # noqa: E402,F401
...
from src.data.alternative.composite_signal_archive import (  # noqa: E402,F401
...
_CONVICTION_TIER_RANK = _alt_data_composite_signals._CONVICTION_TIER_RANK

router.include_router(_alt_data_composite_signals.router)
