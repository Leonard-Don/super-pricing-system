...
(async () => {
  ...
  try {
    ...
    await comparePanel.getByText(/^(Evidence Alignment|证据共振)$/).waitFor({ state: 'visible', timeout: 60000 });
    await comparePanel.getByText(/^(Analysis Window|分析窗口)$/).waitFor({ state: 'visible', timeout: 60000 });
    await comparePanel.getByText(/^(Governance Overlay|治理覆盖层)$/).waitFor({ state: 'visible', timeout: 60000 });
    await comparePanel.getByText(/^(Policy Execution|政策执行)$/).waitFor({ state: 'visible', timeout: 60000 });
    await comparePanel.getByText(/^(Source Mode|来源治理)$/).waitFor({ state: 'visible', timeout: 60000 });
    ...
    await comparePanel.getByText('证据共振', { exact: false }).first().waitFor({ state: 'visible', timeout: 60000 });
