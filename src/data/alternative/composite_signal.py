...
# ---------------------------------------------------------------------------
# Relocated to composite_signal_cluster_aware.py (zero-behavior-change split).
# All symbols are re-exported here so external callers remain unaffected.
#
# ``_resolve_cluster_membership`` is kept here because tests monkeypatch it on
# this module's namespace; the new module imports it back from here so the
# monkeypatch is effective regardless of which module calls the function.
...
from .composite_signal_cluster_aware import (  # noqa: E402
