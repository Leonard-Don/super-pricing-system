...
function CrossMarketResultsSection({ results, selectedTemplate, meta, quality }) {
  const correlationColumns = useMemo(() => buildCorrelationColumns(results), [results]);

  const contributionColumns = useMemo(() => buildContributionColumns(), []);
  ...
  const executionBatchColumns = useMemo(() => buildExecutionBatchColumns(), []);
