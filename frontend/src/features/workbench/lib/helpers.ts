// ---------------------------------------------------------------------------
// helpers — ported from
// frontend/src/components/research-playbook/_helpers.js
// ---------------------------------------------------------------------------

import { buildWorkbenchViewFingerprint } from './workbenchViewFingerprint';
...
export const toPercent = (value: unknown, digits = 1): string =>
  `${(Number(value || 0) * 100).toFixed(digits)}%`;

export const toSignedPercent = (value: unknown, digits = 1): string => {
  const numeric = Number(value || 0);
  return `${numeric > 0 ? '+' : ''}${(numeric * 100).toFixed(digits)}%`;
};

export const toSignedPercentPoints = (value: unknown, digits = 1): string => {
...
export const WORKBENCH_REFRESH_LABELS: Record<string, string> = {
  high: '建议更新',
  medium: '建议复核',
  low: '继续观察',
};

export const WORKBENCH_TYPE_LABELS: Record<string, string> = {
  pricing: 'Pricing',
...
export const WORKBENCH_SNAPSHOT_VIEW_LABELS: Record<string, string> = {
  filtered: '带筛选视角快照',
  scoped: '带任务焦点快照',
};

export const WORKBENCH_REASON_LABELS: Record<string, string> = {
...
// ---------------------------------------------------------------------------
// formatResearchSource  (co-ported from utils/researchContext.js)
// ---------------------------------------------------------------------------

export const formatResearchSource = (source = ''): string => {
...
export const buildWorkbenchViewContext = (context: Record<string, unknown> = {}): Record<string, unknown> | null => {
...
export const buildPricingAction = (symbol: string, source: string, note: string) =>
  symbol
    ? {
        label: '打开定价剧本',
        target: 'pricing',
...
export const buildCrossMarketAction = (template: string, source: string, note: string) =>
  template
    ? {
        label: '打开跨市场剧本',
        target: 'cross-market',
...
export const buildHighlights = (playbook: Record<string, unknown> | null = {}, fallback: string[] = []): string[] => {
  const safePlaybook = playbook || {};
  const highlights = [
    ...((safePlaybook.warnings as string[]) || []),
    ...(fallback || []),
  ].filter(Boolean);
...
// ---------------------------------------------------------------------------
// detectMacroCue / isMacroSensitiveSector / hasProxyFactorInputs
// ---------------------------------------------------------------------------

export const detectMacroCue = (texts: string[] = []): boolean =>
  /政策|宏观|能源|电力|电网|地缘|供应|供给|库存|物流|利率|关税|算力|火电|核电/i.test(
    texts.map((item) => compactText(item)).join(' '),
  );

export const isMacroSensitiveSector = (sector = ''): boolean =>
  /energy|utilities|industrials|materials/i.test(String(sector || '').trim());

export const hasProxyFactorInputs = (factorModel: Record<string, unknown> = {}): boolean =>
  Boolean((factorModel?.factor_source as Record<string, unknown>)?.is_proxy || (factorModel?.five_factor_source as Record<string, unknown>)?.is_proxy);

// ---------------------------------------------------------------------------
// deriveCrossMarketSignal
// ---------------------------------------------------------------------------

export const deriveCrossMarketSignal = (symbol: string, pricingResult: Record<string, unknown> = {}): {
  shouldCrossMarket: boolean;
  reasons: string[];
  macroCue: boolean;
  sector: string;
} => {
  ...
  const texts = [
    ...
    String(gap.severity_label ?? ''),
    ...drivers.map((item) => String(item?.description ?? '')),
    String((implications?.factor_alignment as Record<string, unknown>)?.summary ?? ''),
    String(implications?.primary_view ?? ''),
    ...insights,
  ];
  const reasons: string[] = [];
  const confidence = implications?.confidence;
  const riskLevel = implications?.risk_level;
  const alignmentStatus = (implications?.factor_alignment as Record<string, unknown>)?.status;
  const severity = gap?.severity;
  const sector = String((valuation?.comparables as Record<string, unknown>)?.sector ?? (valuation?.fundamentals as Record<string, unknown>)?.sector ?? '');
  ...
  if (['moderate', 'extreme', 'unknown'].includes(String(severity ?? ''))) {
  ...
  const supportingSignals = [macroCue, proxyInputs, isMacroSensitiveSector(sector), ['moderate', 'extreme', 'unknown'].includes(String(severity ?? '')), POWER_SYMBOLS.has(symbol) || AI_SYMBOLS.has(symbol)].filter(Boolean).length;
...
export const recommendTemplateForSymbol = (symbol = '', texts: string[] = []): string => {
...
export const describeConfidence = (level = ''): string => {
  const mapping: Record<string, string> = {
    low: '低',
    medium: '中',
    high: '高',
