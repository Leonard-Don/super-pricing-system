...
| **定价实验台 (Quant Lab)** | 🧪 | 估值历史 · 自定义因子 · 内部运行支撑；策略/回测/行业/实时信号为迁移候选 |
...
| 🧪 定价实验台 | `http://localhost:3100?view=quantlab` | 估值历史 · 自定义因子 · 运行支撑 · 迁移候选标注 |
...
4. 最后进入 **定价实验台**，核对估值实验、因子表达式和内部运行状态；交易策略、回测、行业轮动和实时信号类能力只作为迁移候选入口保留
...
- **迁移候选** — 策略优化 · 回测增强 · 风险归因 · 行业轮动 · 实时信号验证，继续开发时归 `quant-trading-system`
...
| `/quant-lab/*` | 🧪 定价实验台 | 估值实验 · 因子表达式 · 内部任务/告警；交易类实验仅作迁移候选入口 |
